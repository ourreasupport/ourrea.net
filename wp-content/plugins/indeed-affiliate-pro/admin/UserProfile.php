...
class UserProfile
{
    ...
    public function page( $tab='' )
    {
        ...
        $this->affiliate_id = empty( $_GET['affiliate_id'] ) ? 0 : esc_sql( $_GET['affiliate_id'] );
        $uid = $indeed_db->get_uid_by_affiliate_id( $this->affiliate_id );
		    $this->uid = $uid;
        require_once UAP_PATH . 'classes/MLM_Get_All_Children.class.php';
        $mlm = new \MLM_Get_All_Children( $this->affiliate_id);
        ...
        $data = [
                  ...
                  'description'         => get_user_meta( $uid, 'description', true ),
				          'role'				        => $wp_roles->roles[$indeed_db->get_user_first_role( $uid )]['name'],
        ...
        echo $view->setTemplate( UAP_PATH . 'admin/views/user_profile.php' )
                  ->setContentData( $data )
                  ->getOutput();
    }
    ...
    private function printMlm( $affiliateId=0, $affiliateUsername='' )
    {
        global $indeed_db;
        require_once UAP_PATH . 'classes/MLM_Get_All_Children.class.php';
        $children_object = new \MLM_Get_All_Children($affiliateId);
        $data = [
