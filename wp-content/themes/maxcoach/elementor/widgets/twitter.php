...
class Widget_Twitter extends Base {
	...
	public function get_title() {
		return esc_html__( 'Twitter', 'maxcoach' );
	}
	...
	protected function _register_controls() {
		...
		$this->add_control( 'consumer_key', [
			'label' => esc_html__( 'Consumer Key', 'maxcoach' ),
			'type'  => Controls_Manager::TEXT,
		] );

		$this->add_control( 'consumer_secret', [
			'label' => esc_html__( 'Consumer Secret', 'maxcoach' ),
			'type'  => Controls_Manager::TEXT,
		] );

		$this->add_control( 'access_token', [
			'label' => esc_html__( 'Access Token', 'maxcoach' ),
			'type'  => Controls_Manager::TEXT,
		] );

		$this->add_control( 'access_token_secret', [
			'label' => esc_html__( 'Access Token Secret', 'maxcoach' ),
			'type'  => Controls_Manager::TEXT,
		] );

		$this->add_control( 'username', [
			'label' => esc_html__( 'Twitter Username', 'maxcoach' ),
			'type'  => Controls_Manager::TEXT,
		] );
		...
		$this->add_control( 'show_date', [
				'label'        => esc_html__( 'Show date', 'maxcoach' ),
				'type'         => Controls_Manager::SWITCHER,
				'label_on'     => esc_html__( 'Yes', 'maxcoach' ),
				'label_off'    => esc_html__( 'No', 'maxcoach' ),
				'return_value' => '1',
			]
		);
			...
			<?php foreach ( $tweets as $tweet ) : ?>
				...
				<div class="tweet">
					...
					<?php if ( '1' === $settings['show_date'] ) : ?>
						<span
							class="tweet-date"><?php echo wp_date( 'F d, Y', strtotime( $tweet['created_at'] ) ); ?></span>
					<?php endif; ?>
				</div>
			<?php endforeach; ?>
		</div>
