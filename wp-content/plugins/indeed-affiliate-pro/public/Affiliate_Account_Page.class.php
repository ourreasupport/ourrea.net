	...
	class Affiliate_Account_Page{
		...
		private function print_top_messages(){
			...
			if (empty($payment_settings['is_active']) && empty($this->public_extra_settings['uap_hide_payments_warnings'])){
				...
				$data['warning_messages'][] = __("You didn't submit any Payout Method. You can complete setup ", 'uap'). '<a href="'.$data['payment_url'].'">'. __('here', 'uap').'</a>';
			}
		...
		private function run_visits(){
			...
			$limit = 10;
			require UAP_PATH . 'classes/Indeed_Pagination.class.php';
		...
		private function run_payment_settings(){
			...
			if (!empty($_POST['save_settings']) && !empty($_POST['uap_payment_settings_nonce']) && wp_verify_nonce( $_POST['uap_payment_settings_nonce'], 'uap_payment_settings_nonce' ) ){
				if ($_POST['uap_affiliate_payment_type']=='stripe_v2'){
					...
					require UAP_PATH . 'classes/Uap_Stripe_V2.class.php';
		...
		private function print_custom_affiliate_slug(){
			...
			switch($data['slug_condition_rule']){
				case 1:
						$data['slug_condition_rule'] = __(' and must contains charatects and digits', 'uap');
						break;
				case 2: $data['slug_condition_rule'] = __(' and must contains one uppercase letter, charactes and digits', 'uap');
						break;
				default:
		...
		private function print_mlm_page(){
			...
			$this->print_top_messages();
			require UAP_PATH . 'classes/MLM_Get_All_Children.class.php';
		...
		private function run_simple_links(){
			...
			if ( isset( $_GET['message'] ) ){
				if ( $_GET['message']==0 ){
					$data['err'] = __('Your Referrer Link can not be registered.', 'uap');
				} else if ($_GET['message']==-1){
					$data['err'] = __('This Referrer Link already exist. Please submit a different one.', 'uap');
				}
			}
