...
// 				add_action( 'learn-press/course-item-content', array( $this, 'maybe_restrict_content' ), - 10 );
			...
			if ( $course_items ) {
				foreach ( $course_items as $item_id ) {
					if ( ! $drip_items ) {
						...
						if ( ! isset( $new_drip[ $item_id ]['prerequisite'] ) ) {
		...
		public function update_items() {
            ...
            <!-- add notice-->
            <div class="settings-updated updated">
                <p><?php _e( 'Settings saved', 'learnpress-content-drip' ); ?></p>
            </div>
			<?php
		}
				...
				if ( LP_QUIZ_CPT == $item_type && 'passed' !== $user->get_item_grade( $deps_item_id, $course->get_id() ) ) {
					...
					switch ( $args['type'] ) {
						...
						case 'interval':
							...
							break;
						default:
							$open_time = new LP_Datetime( $end_time->getTimestamp() );
							break;
					}
				}
