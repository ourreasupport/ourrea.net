	...
	class Uap_Main_Admin{
		...
		*/
		...
		private function check_system(){
			...
			if ( !$wp_cron ){
				$this->error_messages[] = __('Crons are disabled on your WordPress Website. Some functionality and processes may not work properly.', 'uap');
			}
			global $indeed_db;

			/// curl
			if ( !function_exists('curl_version') || !curl_version() ){
					$this->error_messages[] = __('cURL is not working or is disabled on your Website environment. Please contact your Hosting provider.', 'uap');
			}
			...
			if ( !$uap_test_cookie || !isset($_COOKIE['uap_test_cookie']) ){
					...
					if (isset($_COOKIE['uap_test_cookie'])) {
							...
							$this->error_messages[] = __('Cookies are not stored or setcookie() PHP function is disabled on your Website environment.', 'uap');
			...
			if ( $indeed_db->is_magic_feat_enable('paypal') ){
				...
				if ( empty( $temp_array['uap_paypal_sandbox'] ) ){
					if ( empty( $temp_array['uap_paypal_client_id'] ) || empty( $temp_array['uap_paypal_client_secret'] ) ){
						$this->error_messages[] = __('PayPal Payout module is not set properly!', 'uap');
					}
				} else {
			...
			if ( $indeed_db->is_magic_feat_enable('stripe') ){
				...
				if ( empty( $temp_array['uap_stripe_sandbox'])){
					if ( empty( $temp_array['uap_stripe_secret_key'] ) || empty($temp_array['uap_stripe_publishable_key'] ) ){
						$this->error_messages[] = __('Stripe Payout module is not set properly!', 'uap');
					}
				} else {
			...
			if ( $indeed_db->affiliates_with_no_rank_exists() ){
				$this->error_messages[] = __('Some Affiliates users do not have assigned a Rank and they may not be rewarded. Check your stage ', 'uap') . ' <a href="' . admin_url('admin.php?page=ultimate_affiliates_pro&tab=affiliates') . '" target="_blank">' . __('here', 'uap') . '</a>';
			}
			...
			if ( !empty($functionsErrors) ){
					$this->error_messages[] = __('Following functions: ', 'uap') . implode( ', ', $functionsErrors )
					. __( ' are disabled on your Website environment. Avatar feature may not work properly. Please contract your Hosting provider.', 'uap');
			}
		}
		...
		private function printFopenError()
		{
				...
				if ( !$allow ){
					echo '<div class="uap-error-global-dashboard-message"><strong>' . __("'allow_url_fopen' directive is disabled. In order for Ultimate Affiliate Pro to work properly this directive has to be set 'on'. Contact your hosting provider for more details.", 'uap') . ' </strong></div>';
				}
		}
		...
		private function print_head($tab){
			...
			$data['menu_items'] = array(
											...
											'visits' => __('Clicks', 'uap'),
											'referrals' => __('Referrals', 'uap'),
											'payments' => __('Payments', 'uap'),
											'magic_features' => __('Extensions', 'uap'),
			...
			if ($data['subtab']=='list'){
				...
				/* else if (!empty($_POST['delete_rank'])){
					if ($indeed_db->ranks_get_count()>1){
						...
						$data['alert_message'] = __('You cannot have less than one rank.', 'uap');
					}
				}
				*/
		...
		private function print_visits(){
			...
			if (!empty($_REQUEST['affiliate_id'])){
				...
				$data['subtitle'] = __('View Clicks for', 'uap') . " $full_name ($username)";
			}
		...
		private function info_affiliate_bar()
		{
				...
				require_once $this->admin_view_path . 'info_affiliate_bar.php';
		}
		...
		private function product_links()
		{
				...
				require_once $this->admin_view_path . 'product_links.php';
		}
		...
		public function add_style_scripts(){
			...
			wp_enqueue_media();
			wp_register_script('uap_admin_js', UAP_URL . 'assets/js/admin-functions.js', array(), 1.1 );
			...
			if ( $pagenow == 'plugins.php' ){
					...
					wp_enqueue_script( 'indeed_sweetalert_js', UAP_URL . 'assets/js/sweetalert.js' );
			...
			if ($is_plugin_page){
				...
				//wp_register_script('uap_public', UAP_URL . 'assets/js/public-functions.js', array(), 1.1 );
				wp_register_script('uap_public', UAP_URL . 'assets/js/public-functions.min.js', array(), 1.1 );
