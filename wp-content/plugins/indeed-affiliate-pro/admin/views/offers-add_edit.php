					...
					<div class="row">
						<div class="col-xs-4">
							...
							<div class="input-group" style="margin-top:10px;">
								<span class="input-group-addon js-uap-product-label" id="basic-addon1">
											<?php if ( $data['metas']['source'] == 'woo' ):?>
													<?php _e( 'Products/Categories', 'uap' );?>
											<?php else :?>
													<?php _e('Products', 'uap');?>
											<?php endif;?>
											</span>
								...
								<div id="uap_username_search_tags"><?php
									if (!empty($data['metas']['affiliates'])){
										foreach ($data['metas']['affiliates'] as $value){
											...
											?>
											<div id="<?php echo $id;?>" class="uap-tag-item"><?php echo $data['affiliates']['username'][$value];?><div class="uap-remove-tag" onclick="uapRemoveTag('<?php echo $value;?>', '#<?php echo $id;?>', '#usernames_search_hidden');" title="<?php _e('Removing tag', 'uap');?>">x</div></div>
											<?php
											}
										}
									}
...
jQuery(document).ready(function(){
	jQuery('#the_source').on('change', function(){
		...
		console.log(  uap_offer_source )
		if ( uap_offer_source == 'woo' ){
				jQuery( '.js-uap-product-label').html( "<?php _e( 'Products/Categories', 'uap' );?>" );
		} else {
				jQuery( '.js-uap-product-label').html( "<?php _e( 'Products', 'uap' );?>" );
		}
		...
		select: function( event, ui ) {
			...
			if ( typeof ui.item.is_category != 'undefined' ){
					...
					for ( var i = 0; i < ui.item.children.length; i++ ){
							if (!contains(terms, ui.item.children[i].id )){
