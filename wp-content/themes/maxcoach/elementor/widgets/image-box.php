...
class Widget_Image_Box extends Base {
	...
	public function get_title() {
		return esc_html__( 'Image Box', 'maxcoach' );
	}
	...
	private function add_image_box_section() {
		$this->start_controls_section( 'image_section', [
			'label' => esc_html__( 'Image Box', 'maxcoach' ),
		] );
		...
		$this->add_control( 'image_position', [
			'label'     => esc_html__( 'Image Position', 'maxcoach' ),
		...
		$this->add_control( 'title_size', [
			'label'   => esc_html__( 'HTML Tag', 'maxcoach' ),
			'type'    => Controls_Manager::SELECT,
			'options' => [
				'h1'   => 'H1',
				'h2'   => 'H2',
		...
		$this->add_control( 'description_text', [
			...
			'default'     => esc_html__( 'Click edit button to change this text. Lorem ipsum dolor sit amet, consectetur adipiscing elit. Ut elit tellus, luctus nec ullamcorper mattis, pulvinar dapibus leo.', 'maxcoach' ),
	...
	private function add_image_style_section() {
		...
		$this->add_responsive_control( 'image_wrap_height', [
			...
			'selectors' => [
				'{{WRAPPER}} .maxcoach-image' => 'min-height: {{SIZE}}{{UNIT}};',
			],
		] );

		$this->add_responsive_control( 'image_space_top', [
			'label'     => esc_html__( 'Offset Top', 'maxcoach' ),
			'type'      => Controls_Manager::SLIDER,
			'range'     => [
				'px' => [
					'min' => 0,
	...
	private function add_content_style_section() {
		...
		$this->start_controls_tabs( 'box_caption_colors', [
			'condition' => [
				'style' => [ '05' ],
			],
		] );

		$this->start_controls_tab( 'box_caption_colors_normal', [
			'label' => esc_html__( 'Normal', 'maxcoach' ),
		] );

		$this->add_group_control( Group_Control_Background::get_type(), [
			'name'     => 'box_caption',
			'selector' => '{{WRAPPER}} .maxcoach-box .content',
		] );

		$this->add_group_control( Group_Control_Advanced_Border::get_type(), [
			'name'     => 'box_caption_border',
			'selector' => '{{WRAPPER}} .maxcoach-box .content',
		] );
		...
		$this->add_control( 'box_caption_line_hr', [
		...
		$this->add_group_control( Group_Control_Background::get_type(), [
			...
			'selector' => '{{WRAPPER}} .maxcoach-box .content:after',
		] );

		$this->end_controls_tab();

		$this->start_controls_tab( 'box_caption_colors_hover', [
			'label' => esc_html__( 'Hover', 'maxcoach' ),
		] );

		$this->add_group_control( Group_Control_Background::get_type(), [
			'name'     => 'box_caption_hover',
			'selector' => '{{WRAPPER}} .maxcoach-box .content:before',
		] );

		$this->add_group_control( Group_Control_Advanced_Border::get_type(), [
			'name'     => 'box_caption_hover_border',
			'selector' => '{{WRAPPER}} .maxcoach-box .content:before',
		] );
	...
	protected function render() {
		...
		$this->add_render_attribute( 'wrapper', 'class', 'tm-image-box maxcoach-box' );
	...
	protected function _content_template() {
		...
		?>
		<#
		view.addRenderAttribute( 'wrapper', 'class', 'tm-image-box maxcoach-box' );
		...
		if ( settings.image.url ) {
			...
			imageHTML = '<div class="maxcoach-image image"><img ' + view.getRenderAttributeString( 'image' ) + ' /></div>';
		}
		#>
	...
	private function print_description (array $settings) {
		...
		<div <?php $this->print_render_attribute_string('description_text'); ?>>
			<?php echo wp_kses($settings['description_text'], 'maxcoach-default'); ?>
		</div>
		<?php
	}
}
