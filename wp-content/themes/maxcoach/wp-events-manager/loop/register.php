	...
	<ul class="entry-event-info">
		<li class="meta-price">
			<span class="meta-label">
				...
				<?php esc_html_e( 'Cost:', 'maxcoach' ); ?>
			</span>
			<span
				class="meta-value">
				<span
					class="event-price"><?php printf( '%s', $event->is_free() ? esc_html__( 'Free', 'maxcoach' ) : wpems_format_price( $event->get_price() ) ) ?></span>
			</span>
		</li>
		<li class="total">
			<span class="meta-label">
				<i class="meta-icon far fa-user-friends"></i>
				<?php esc_html_e( 'Total Slot:', 'maxcoach' ); ?>
			</span>
		...
		<li class="booking_slot">
			<span class="meta-label">
				...
				<?php esc_html_e( 'Booked Slot:', 'maxcoach' ); ?>
			</span>
	...
	<?php if ( $can_register ): ?>
		<?php if ( is_user_logged_in() ) { ?>
			...
			if ( $registered_time && wpems_get_option( 'email_register_times' ) === 'once' && $event->is_free() ) { ?>
				<p class="event-register-message"><?php esc_html_e( 'You have registered this event before.', 'maxcoach' ); ?></p>
			<?php } else { ?>
				<a class="event_register_submit event_auth_button event-load-booking-form"
				   data-event="<?php echo esc_attr( get_the_ID() ) ?>"><?php esc_html_e( 'Book Now', 'maxcoach' ); ?></a>
			<?php } ?>
		<?php } else { ?>
			<a href="<?php echo esc_url( wpems_login_url() ); ?>" class="tm-button style-flat">
				<div class="button-content-wrapper">
					<div class="button-text"><?php esc_html_e( 'Book Now', 'maxcoach' ); ?></div>
				</div>
			</a>
			<p class="event-register-message"><?php echo sprintf( __( 'You must <a href="%s">login</a> before register event.', 'maxcoach' ), wpems_login_url() ); ?></p>
		<?php } ?>
	<?php else: ?>
		<p class="tp-event-notice error"><?php echo esc_html__( 'This event has expired', 'maxcoach' ); ?></p>
	<?php endif; ?>
