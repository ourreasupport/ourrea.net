    ...
    class Wt_Smart_Coupon_Auto_Coupon {
        ...
        function get_all_auto_coupons() {
            ...
            if( ! is_array( $this->autocoupons ) ) {
                ...
                $coupon_items = $wpdb->get_results( $wpdb->prepare
                (
                    "SELECT DISTINCT `ID` as coupon_id FROM $wpdb->posts as P INNER JOIN $wpdb->postmeta AS PM1 ON (P.`ID` = PM1.`post_id`) WHERE P.`post_type` = 'shop_coupon' AND P.`post_status` = 'publish' AND PM1.`meta_key` = '_wt_make_auto_coupon' AND PM1.`meta_value` = 1  ORDER BY P.`post_date` DESC LIMIT %d" , $auto_coupon_limit
                )
                );
