...
{$.fbuilder.showHideDep({'formIdentifier':evt.data.identifier,'fieldIdentifier':evt.target.id});});setTimeout(function(){$.fbuilder.showHideDep({'formIdentifier':opt.identifier});},50);try
...
{var regExp=new RegExp((parseInt(name,10)==name)?'fieldname'+name+'_':name+'_',i);if(name in fieldsIndex)return items[fieldsIndex[name]];for(var i in items)
...
{raw=raw||false;var e=$('[id="'+this.name+'"]:not(.ignore)'),p=$.fbuilder.parseValStr(e.val(),raw);if(e.length)return($.isNumeric(p))?'"'+p+'"':p;return 0;},setVal:function(v)
