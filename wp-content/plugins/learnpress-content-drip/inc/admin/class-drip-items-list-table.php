			...
			switch ( $column_name ) {
				...
				case 'prerequisite':
					...
					if ( $course_items ) {
						...
						?>
						<select class="drip-prerequisite-items" name="item-delay[<?php echo esc_attr( $item ); ?>][prerequisite][]" multiple="multiple">
							<?php
							unset( $course_items[ $index ] );
							foreach ( $course_items as $course_item ) {
								$selected_item = isset( $drip_items[ $item ]['prerequisite'] ) ? $drip_items[ $item ]['prerequisite'] : $pre_item_id; ?>
								<option value="<?php echo esc_attr( $course_item ); ?>" <?php echo is_array( $selected_item ) ? ( in_array( $course_item, $selected_item ) ? 'selected' : '' ) : $selected_item == $course_item ? 'selected' : ''; ?>><?php echo get_the_title( $course_item ); ?></option>
							<?php }
							?>
						</select>
					<?php }
				...
				case 'delay':
					...
					<div class="item-delay <?php echo $config['type']; ?>">
						...
						<div class="delay-specific">
							<input type="text" class="delay-specific-datetimepicker" name="item-delay[<?php echo $item; ?>][date]"
							       value="<?php echo is_string( $config['date'] ) ? $config['date'] : date( get_option( 'date_format' ) . ' H:i', $config['date'] ); ?>">
						</div>
					</div>
					<?php
					break;
