	...
	public function get_settings() {
		...
		if ( $payment_gateways ) {
			foreach ( $payment_gateways as $payment_gateway ) {
				$title = sprintf(
					__( 'This payment is %s, please click the link beside to enable/disable.', 'learnpress-woo-payment' ),
		...
		return
			array(
				array(
					...
					'desc'    => __( 'If <strong>WooCommerce Payment</strong> is enabled you can not use other payments provided by LearnPress', 'learnpress-woo-payment' )
				),
				array(
				...
				array(
					...
					'default' => '',
					'type'    => 'html',
					'desc'    => __( 'List of all available payment gateways installed and activated for WooCommerce. Click on a payment method to go to <strong>WooCommerce Payment</strong> settings.', 'learnpress-woo-payment' ),
					'html'    => $available_payment_html ? sprintf( '<ul class="learn-press-woo-payments">%s</ul>', $available_payment_html ) : '',
				)
			);
	}
            ...
            <li class="learn_press_woo_payment_methods">
                <label>
                    <input id="payment_method_<?php echo $gateway->id; ?>" type="radio" class="input-radio"
                           ...
                           data-method="<?php echo esc_attr( $gateway->id ); ?>" <?php checked( LP()->session->get( 'chosen_payment_method' ) == $gateway->id, true ); ?>
                           data-order_button_text="<?php echo esc_attr( $gateway->order_button_text ); ?>"/>
					<?php echo( $gateway->get_title() ); ?>
                </label>
				<?php if ( $payment_form = $gateway->get_description() ) { ?>
