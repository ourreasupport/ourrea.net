...
<div class="uap-wrapper">
	...
	<?php if (!empty($data['listing_items'])):?>
		...
		<?php $i = 0; foreach ($data['listing_items'] as $k=>$array):?>
			<tr onmouseover="uapDhSelector('#aff_<?php echo $array['id'];?>', 1);" onmouseout="uapDhSelector('#aff_<?php echo $array['id'];?>', 0);" class="<?php if ($i%2==0) echo 'alternate';?>">
				<th><?php
					echo $array['code'];?>
					<div id="<?php echo 'aff_' . $array['id'];?>" style="visibility: hidden;">
						<a href="<?php echo $data['url-add_edit'] . '&add_edit=' . $array['code'];?>"><?php _e('Edit', 'uap');?></a> | <a onclick="uapDeleteFromTable(<?php echo $array['id'];?>, 'Coupon', '#delete_coupons', '#form_coupons');" href="javascript:return false;" style="color: red;"><?php _e('Delete', 'uap');?></a>
					</div>
				</th>
