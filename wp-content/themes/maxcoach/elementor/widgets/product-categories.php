...
class Widget_Product_Categories extends Base {
	...
	public function get_title() {
		return esc_html__( 'Product Categories', 'maxcoach' );
	}
	...
	public function get_script_depends() {
		return [ 'maxcoach-group-widget-grid' ];
	}
	...
	private function add_query_section() {
		...
		$this->add_control( 'source', [
			...
			'type'        => Controls_Manager::SELECT,
			'options'     => [
				''                      => esc_html__( 'Show All', 'maxcoach' ),
				'by_id'                 => esc_html__( 'Manual Selection', 'maxcoach' ),
				'by_parent'             => esc_html__( 'By Parent', 'maxcoach' ),
				'current_subcategories' => esc_html__( 'Current Subcategories', 'maxcoach' ),
			],
			'label_block' => true,
		] );
		...
		$parent_options = [ '0' => esc_html__( 'Only Top Level', 'maxcoach' ) ] + $options;
		$this->add_control(
			'parent', [
			'label'     => esc_html__( 'Parent', 'maxcoach' ),
			'type'      => Controls_Manager::SELECT,
			'default'   => '0',
		...
		$this->add_control( 'hide_empty', [
			'label'     => esc_html__( 'Hide Empty', 'maxcoach' ),
			'type'      => Controls_Manager::SWITCHER,
			'default'   => 'yes',
			'label_on'  => 'Hide',
			'label_off' => 'Show',
		...
		$this->add_control( 'number', [
			'label'   => esc_html__( 'Categories Count', 'maxcoach' ),
			'type'    => Controls_Manager::NUMBER,
			'default' => '4',
		] );
		...
		$this->add_control( 'orderby', [
			...
			'default' => 'name',
			'options' => [
				'name'        => esc_html__( 'Name', 'maxcoach' ),
				'slug'        => esc_html__( 'Slug', 'maxcoach' ),
				'description' => esc_html__( 'Description', 'maxcoach' ),
				'count'       => esc_html__( 'Count', 'maxcoach' ),
			],
		] );

		$this->add_control( 'order', [
			'label'   => esc_html__( 'Order', 'maxcoach' ),
			'type'    => Controls_Manager::SELECT,
			'default' => 'desc',
			'options' => [
				'asc'  => esc_html__( 'ASC', 'maxcoach' ),
				'desc' => esc_html__( 'DESC', 'maxcoach' ),
			],
		] );
	...
	private function add_image_style_section() {
		$this->start_controls_section( 'images_style_section', [
			'label' => esc_html__( 'Images', 'maxcoach' ),
	...
	protected function render() {
		...
		$this->add_render_attribute( 'grid-wrapper', 'class', 'maxcoach-grid-wrapper maxcoach-product-categories' );

		$this->add_render_attribute( 'content-wrapper', 'class', 'maxcoach-grid lazy-grid' );

		if ( 'metro' === $settings['layout'] ) {
			$this->add_render_attribute( 'grid-wrapper', 'class', 'maxcoach-grid-metro' );
		}
	...
	private function print_grid() {
		$settings   = $this->get_settings_for_display();
		$image_size = \Maxcoach_Image::elementor_parse_image_size( $settings, '600x600' );
		...
		foreach ( $this->terms as $term ) {
			...
			?>
			<div class="grid-item">
				<a href="<?php echo esc_url( $link ); ?>" class="product-cat-wrapper maxcoach-box cat-link link-secret">
	...
	private function print_image( $thumbnail_id, $image_size ) {
		...
		<?php if ( ! empty( $thumbnail_id ) ) : ?>
			<?php \Maxcoach_Image::the_attachment_by_id( array(
				'id'   => $thumbnail_id,
				'size' => $image_size,
			) ); ?>
		<?php else: ?>
