...
if ( ( ! defined( 'INSIGHT_CORE_SHOW_EXPORT_PAGE' ) || ! INSIGHT_CORE_SHOW_EXPORT_PAGE ) && ! WP_DEBUG && ( ! isset( $_GET['debug'] ) || 'true' != $_GET['debug'] ) ) {
	return;
}
...
class InsightCore_Export {
	...
	function export_media_packages() {
		...
		$dst = WP_CONTENT_DIR . INSIGHT_CORE_DS . INSIGHT_CORE_THEME_SLUG . '-' . $_POST['demo'];
	...
	function export_media_packages_placeholder() {
		$src = WP_CONTENT_DIR . '/uploads';
		$dst = WP_CONTENT_DIR . INSIGHT_CORE_DS . INSIGHT_CORE_THEME_SLUG . '-' . $_POST['demo_placeholder'];
