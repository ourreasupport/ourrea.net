...
if (!class_exists('Uap_Db')){
	class Uap_Db{
		...
		*/
		public function get_all_prefixes()
		{
				...
				if (is_multisite() ){
						if ( is_network_admin() ){
							...
							return [ $wpdb->baseprefix . get_current_blog_id() . '_' ];
						}
        }
        return $data;
    }
		...
		{
				global $wpdb;
				$indexList = $wpdb->get_results( "SHOW INDEX FROM {$wpdb->prefix}uap_visits;" );
				if ( !$indexList ){
						return;
				}
				...
				$wpdb->query( "CREATE INDEX idx_uap_visits_affiliate_id ON {$wpdb->prefix}uap_visits(affiliate_id)" );
		}

		private function addUapAffiliatesIndex()
		{
				global $wpdb;
				$indexList = $wpdb->get_results( "SHOW INDEX FROM {$wpdb->prefix}uap_affiliates;" );
				if ( !$indexList ){
						return;
				}
				foreach ( $indexList as $indexObject ){
				...
				$wpdb->query( "CREATE INDEX idx_uap_affiliates_uid ON {$wpdb->prefix}uap_affiliates(uid)" );
		}

		private function addReferralsIndex()
		{
				global $wpdb;
				$indexList = $wpdb->get_results( "SHOW INDEX FROM {$wpdb->prefix}uap_referrals;" );
				if ( !$indexList ){
						return;
				}
				foreach ( $indexList as $indexObject ){
				...
				$wpdb->query( "CREATE INDEX idx_uap_referrals_affiliate_id ON {$wpdb->prefix}uap_referrals(affiliate_id)" );
		}

		private function addPaymentsIndex()
		{
				global $wpdb;
				$indexList = $wpdb->get_results( "SHOW INDEX FROM {$wpdb->prefix}uap_payments;" );
				if ( !$indexList ){
						return;
				}
				foreach ( $indexList as $indexObject ){
				...
				$wpdb->query( "CREATE INDEX idx_uap_payments_affiliate_id ON {$wpdb->prefix}uap_payments(affiliate_id)" );
		}

		private function addMlmRelationIndexes()
		{
				global $wpdb;
				$indexList = $wpdb->get_results( "SHOW INDEX FROM {$wpdb->prefix}uap_mlm_relations;" );
				if ( !$indexList ){
						return;
				}
				$doIt = true;
				...
				if ( $doIt ){
						$wpdb->query( "CREATE INDEX idx_uap_mlm_relations_affiliate_id ON {$wpdb->prefix}uap_mlm_relations(affiliate_id)" );
				}
				...
				if ( $doIt ){
						$wpdb->query( "CREATE INDEX idx_uap_mlm_relations_parent_affiliate_id ON {$wpdb->prefix}uap_mlm_relations(parent_affiliate_id)" );
				}
		}
		...
		{
				global $wpdb;
				$indexList = $wpdb->get_results( "SHOW INDEX FROM {$wpdb->prefix}uap_affiliate_referral_users_relations;" );
				if ( !$indexList ){
						return;
				}
				$doIt = true;
				...
				if ( $doIt ){
						$wpdb->query( "CREATE INDEX idx_uap_affiliate_referral_users_relations_affiliate_id ON {$wpdb->prefix}uap_affiliate_referral_users_relations(affiliate_id)" );
				}
				...
				if ( $doIt ){
						$wpdb->query( "CREATE INDEX idx_uap_affiliate_referral_users_relations_referral_wp_uid ON {$wpdb->prefix}uap_affiliate_referral_users_relations(referral_wp_uid)" );
				}
		}
		...
		{
				global $wpdb;
				$indexList = $wpdb->get_results( "SHOW INDEX FROM {$wpdb->prefix}uap_ranks_history;" );
				if ( !$indexList ){
						return;
				}
				foreach ( $indexList as $indexObject ){
				...
				$wpdb->query( "CREATE INDEX idx_uap_ranks_history_affiliate_id ON {$wpdb->prefix}uap_ranks_history(affiliate_id)" );
		}
		...
		{
				global $wpdb;
				$indexList = $wpdb->get_results( "SHOW INDEX FROM {$wpdb->prefix}uap_ref_links;" );
				if ( !$indexList ){
						return;
				}
				foreach ( $indexList as $indexObject ){
				...
				$wpdb->query( "CREATE INDEX idx_uap_ref_links_affiliate_id ON {$wpdb->prefix}uap_ref_links(affiliate_id)" );
		}
			 ...
			 */
			 $insert_array = array(
						'uap_general_register_default_page' => array(
											'title' => __('Become an Affiliate', 'uap'),
						...
						'uap_general_login_default_page' => array(
											'title' => __('Affiliate Login', 'uap'),
											'content' => '[uap-login-form]',
						),
						'uap_general_user_page' => array(
											'title' => __('My Account', 'uap'),
											'content' => '[uap-account-page]',
						),
						'uap_general_lost_pass_page' => array(
											'title' => __('Lost Password', 'uap'),
											'content' => '<p>Please enter your username or email address below. You will receive a link to confirm the request via email. A new generated password will be provided which can be changed later from My Account.</p><p>[uap-reset-password]</p>',
						),
		...
		public function create_demo_banners(){
			 ...
			 */
			 ...
			 foreach ($prefixes as $the_table_prefix){
				 ...
				 $data = $wpdb->get_row("SELECT `id`,`name`,`description`,`url`,`image`,`status`,`DATE` FROM $table;");
				 if (empty($data)){
		...
		public function get_banners(){
			...
			$data = $wpdb->get_results("SELECT `id`,`name`,`description`,`url`,`image`,`status`,`DATE` FROM $table");
		...
		public function notification_type_exists($type='', $table=''){
			...
			if (!empty($type)){
				...
				$q = $wpdb->prepare("SELECT id,type,rank_id,subject,message,pushover_message,pushover_status,status FROM $table WHERE type=%d ", $type);
				$data = $wpdb->get_results($q);
				if (!empty($data)){
					return TRUE;
				}
		...
		public function get_notifications(){
			...
			global $wpdb;
			$table = $wpdb->prefix . "uap_notifications";
			$data = $wpdb->get_results("SELECT id,type,rank_id,subject,message,pushover_message,pushover_status,status FROM $table");
			return (array)$data;
		}
			...
			switch ($type){
				...
				case 'login-messages':
					$arr = array(
									...
									'uap_reset_msg_pass_ok' => 'An e-mail has been sent to You, follow the steps to reset your password.',
									'uap_login_error_email_pending' => 'Your Affiliate E-mail address has not been verified yet',
									'uap_login_error_on_captcha' => 'You have an error on reCaptcha',
								);
				break;
				...
				case 'register-msg':
					$arr = array(
									...
									'uap_register_err_tos' => 'Please accept our Terms&Conditions',
									'uap_register_success_meg' => 'Successfully Registered!',
				...
				case 'account_page':
					$arr = array(	'uap_ap_top_theme' => 'uap-ap-top-theme-2',
									...
									/// TABS SETTINGS
									'uap_tab_overview_title' => __('Dashboard', 'uap'),
									'uap_tab_overview_menu_label' => __('Dashboard', 'uap'),
									'uap_tab_overview_content' => __('', 'uap'),
									'uap_tab_edit_account_title' => __('Edit Your Account', 'uap'),
									'uap_tab_edit_account_menu_label' => __('Edit Your Account', 'uap'),
									'uap_tab_edit_account_content' => '',
									'uap_tab_change_pass_title' => __('Change Your Password', 'uap'),
									'uap_tab_change_pass_menu_label' => __('Change Your Password', 'uap'),
									...
									'uap_tab_banners_title' => __('Creatives', 'uap'),
									'uap_tab_banners_menu_label' => __('Creatives', 'uap'),
									'uap_tab_banners_content' => '',

									'uap_tab_product_links_title' => __('Product Links', 'uap'),
									'uap_tab_product_links_menu_label' => __('Product Links', 'uap'),
									...
									'uap_tab_reports_title' => __('Reports', 'uap'),
									'uap_tab_reports_menu_label' => __('Reports', 'uap'),
...
<p>Affiliates use the affiliate referral link/URL to promote current website or products. Specific affiliates are tracked because their IDs or usernames are appended to their URL, therefore the system can track which affiliate link brought a customer to your website. If the customer successfully completes a conversion (i.e. a sale, or a form submission), a referral will be generated and the affiliate will be awarded a commission.</p>',
									'uap_tab_payments_settings_title' => __('Payout Details', 'uap'),
									'uap_tab_payments_settings_menu_label' => __('Payout Details', 'uap'),
									...
									'uap_tab_custom_affiliate_slug_menu_label' => __('Your Affiliate Slug', 'uap'),
									'uap_tab_custom_affiliate_slug_content' => '',
									'uap_tab_mlm_title' => __('Multi Level Marketing', 'uap'),
									'uap_tab_mlm_menu_label' => __('Multi Level Marketing', 'uap'),
									'uap_tab_mlm_content' => '',
									...
									'uap_tab_simple_links_title' => __('Referrer Links', 'uap'),
									'uap_tab_simple_links_menu_label' => __('Referrer Links', 'uap'),
									'uap_tab_simple_links_content' => '',
									'uap_tab_landing_pages_title' => __('Landing page', 'uap'),
									'uap_tab_landing_pages_menu_label' => __('Landing page', 'uap'),
									...
									'uap_tab_iab_settings_title'				=> __( 'Settings', 'uap' ),
									'uap_tab_iab_settings_menu_label'		=> __( 'Settings', 'uap' ),
									'uap_tab_iab_settings_content'			=> '',
									'uap_tab_iab_settings_icon_code'		=> 'f1de',
									'uap_tab_iab_tips_title'						=> __( 'Help', 'uap' ),
									'uap_tab_iab_tips_menu_label'				=> __( 'Help', 'uap' ),
				...
				case 'woo_account_page':
					$arr = array(
									...
									'uap_woo_account_page_non_affiliate_content' => '<p>' . __('How about to become our affiliate?', 'uap') . '</p>[uap-user-become-affiliate]',
					);
					break;
				case 'bp_account_page':
					$arr = array(
									...
									'uap_bp_account_page_non_affiliate_content' => '<p>' . __('How about to become our affiliate?', 'uap') . '</p>[uap-user-become-affiliate]',
					);
					break;
...
<div><strong>Refuse Referrals :</strong> {refuse_referrals}</div>
				...
				case 'info_affiliate_bar':
					$arr = array(
									...
									'uap_info_affiliate_bar_links_label'												=> __( 'Referrer Links', 'uap' ),
									'uap_info_affiliate_bar_links_get_label'										=> __( 'Get Affiliate Link', 'uap' ),
									'uap_info_affiliate_bar_banner_default_value'								=> '',
									'uap_info_affiliate_bar_banner_section_enabled'							=> 1,
									'uap_info_affiliate_bar_banner_label'												=> __( 'Banner Image', 'uap' ),
									'uap_info_affiliate_bar_social_section_enabled'							=> 1,
									'uap_info_affiliate_bar_social_shortcode'										=> '',
									'uap_info_affiliate_bar_social_label'												=> __( 'Share on', 'uap' ),
									'uap_info_affiliate_bar_stats_general_section_enabled'			=> 1,
									'uap_info_affiliate_bar_stats_personal_section_enabled'			=> 1,
									'uap_info_affiliate_bar_stats_label'												=> __( 'Current Page Stats', 'uap' ),
									'uap_info_affiliate_bar_visits_label'												=> __( 'Visits', 'uap'),
									'uap_info_affiliate_bar_referrals_label'										=> __( 'Referrals', 'uap' ),
									'uap_info_affiliate_bar_insigts_label'											=> __( 'Your Insight:', 'uap' ),
									'uap_info_affiliate_bar_menu_section_enabled'								=> 1,
									'uap_info_affiliate_bar_menu_label'													=> __( 'Settings Menu', 'uap' ),
									'uap_info_affiliate_bar_conversion_rate_label'							=> __(' Conversion Rate', 'uap'),
									'uap_info_affiliate_bar_overall_performance_label' 					=> __('Overall performance:', 'uap'),
					);
					break;
				case 'product_links':
					$arr = array(
		...
		public function get_visits($limit=-1, $offset=-1, $count=FALSE, $order_by='', $order_type='', $where_conditions=array() ){
			...
			if ($count){
				...
				$q = "SELECT v.id,v.ref_hash,v.referral_id,v.affiliate_id,v.campaign_name,v.ip,v.url,v.browser,v.device,v.visit_date,v.status FROM $table v";
				$q .= " INNER JOIN $table_affiliates a ON v.affiliate_id=a.id";
				$q .= " INNER JOIN $table_users u ON a.uid=u.ID";
				$q .= " WHERE 1=1";
				if (!empty($where_conditions)){
		...
		public function get_offers(){
			...
			$data = $wpdb->get_results("SELECT `id`,`name`,`start_date`,`end_date`,`amount_type`,`amount_value`,`settings`,`status` FROM $table;");
			$affiliates_str = '';
			if (!empty($data)){
				foreach ($data as $object){
		...
		public function get_offer_id_by_affiliate_id_and_source($affiliate_id=0, $source=''){
			...
			if ($affiliate_id){
				...
				$data = $wpdb->get_results("SELECT d.id,d.offer_id,d.affiliate_id,d.source,d.products,b.name, b.start_date, b.end_date, b.amount_type, b.amount_value, b.settings, b.status
					 						FROM $table d
		...
		public function get_unpaid_payments_for_affiliate($affiliate_id=0, $limit=-1, $offset=-1, $count=FALSE, $order_by='date', $order_type='DESC', $where_conditions=array()){
			...
			if ($affiliate_id){
				...
				$q = "SELECT `id`,`refferal_wp_uid`,`campaign`,`affiliate_id`,`visit_id`,`description`,`source`,`reference`,`reference_details`,`parent_referral_id`,`child_referral_id`,`amount`,`currency`,`date`,`status`,`payment`
								FROM $table
								WHERE affiliate_id='$affiliate_id'
								AND status='2'
								AND payment='0'";
		...
		public function get_paid_referrals_for_affiliate($affiliate_id=0, $limit=-1, $offset=-1, $count=FALSE, $order_by='date', $order_type='DESC', $where_conditions=array()){
			...
			if ($affiliate_id){
				...
				$table = $wpdb->prefix . 'uap_referrals';
				if ($count){
					$q = "SELECT COUNT(id) as c FROM $table WHERE affiliate_id='$affiliate_id' AND payment='2'";
					if (!empty($where_conditions)){
						foreach ($where_conditions as $condition){
							$q .= " AND " . $condition ;
						}
				...
				$q = "SELECT `id`,`refferal_wp_uid`,`campaign`,`affiliate_id`,`visit_id`,`description`,`source`,`reference`,`reference_details`,`parent_referral_id`,`child_referral_id`,`amount`,`currency`,`date`,`status`,`payment` FROM $table WHERE affiliate_id='$affiliate_id' AND payment='2'";
				if (!empty($where_conditions)){
					foreach ($where_conditions as $condition){
						$q .= " AND " . $condition ;
					}
		...
		public function get_stats_for_payments($affiliate_id=0, $exclude_sources_from_referrals=''){
			...
			$temp_data = $wpdb->get_row("SELECT COUNT(a.id) as c FROM $table as a INNER JOIN $table_b as b ON a.uid=b.ID;");
			...
			$q = "SELECT COUNT(id) as c, COUNT(IF(referral_id != 0,1,null)) d FROM $temp_table WHERE affiliate_id=$affiliate_id";
			$temp_data = $wpdb->get_row($q);
		...
		public function get_last_referrals($limit=5){
			...
			$table = $wpdb->prefix . 'uap_referrals';
			$limit = esc_sql($limit);
			$data = $wpdb->get_results("SELECT `id`,`refferal_wp_uid`,`campaign`,`affiliate_id`,`visit_id`,`description`,`source`,`reference`,`reference_details`,`parent_referral_id`,`child_referral_id`,`amount`,`currency`,`date`,`status`,`payment` FROM $table ORDER BY `date` DESC LIMIT $limit;");
			if ($data){
				foreach ($data as $object){
			...
			if ($data && is_array($data)){
				foreach ($data as $object){
					...
					if (empty($array[$key]['name'])){
							$array[$key]['name'] = __('Deleted Affiliate', 'uap');
					}
					if (isset($where)){
						unset($where);
					}
		...
		public function get_stats_for_reports($time='', $affiliate_id=0){
			...
			$q = "SELECT COUNT(id) as c FROM $table WHERE 1=1 AND referral_id>0";
		 ...
		 public function getReferralsAmountForLastDays( $affiliateId=0, $numberOfDays=0 )
		{
				...
				foreach ( $data as $object ){
						$temporary[ $object->day ] = $object->amount;
				}
				for ( $i=$numberOfDays; $i>0; $i-- ){
						$key = date( 'Y-m-d', time() - $i * 24 * 60 * 60 );
						if ( isset( $temporary[$key] ) ){
		...
		public function get_transation_details($id=0){
			...
			if ($id){
				...
				if ($data){
					$table = $wpdb->prefix . 'uap_referrals';
					$referrals_data = $wpdb->get_results("SELECT `id`,`refferal_wp_uid`,`campaign`,`affiliate_id`,`visit_id`,`description`,`source`,`reference`,`reference_details`,`parent_referral_id`,`child_referral_id`,`amount`,`currency`,`date`,`status`,`payment` FROM $table WHERE id IN (" . $data->referral_ids . ");");
					if ($referrals_data){
		...
		public function get_visits_for_graph($interval='today', $succes_filter='all', $affiliate_id=0){
			...
			global $wpdb;
			$table = $wpdb->prefix . 'uap_visits';
			$q = "SELECT COUNT(id) as value, visit_date FROM $table";
			$q .= " WHERE 1=1";
			if (!empty($start)){
		...
		public function get_referrals_for_graph($interval='today', $status=-1, $affiliate_id=0){
			...
			global $wpdb;
			$table = $wpdb->prefix . 'uap_referrals';
			$q = "SELECT COUNT(id) as value, date FROM $table";
			$q .= " WHERE 1=1";
			if (!empty($start)){
...
update_option('GPL001122334455AA6677BB8899CC000', $code);
return true;
			/*
			 * @param string
			 * @return boolean
		...
		public function get_landing_commissions(){
			 ...
			 */
			 ...
			 $data = $wpdb->get_results("SELECT id,slug,settings,create_date,status FROM $table");
			 if ($data){
			 	foreach ($data as $object){
			 		$array[] = (array)$object;
			 	}
		...
		public function get_coupons_affiliates_pairs(){
			 ...
			 */
			 ...
			 global $wpdb;
			 $table = $wpdb->prefix . 'uap_coupons_code_affiliates';
			 $data = $wpdb->get_results("SELECT `id`,`code`,`affiliate_id`,`type`,`settings`,`status` FROM $table;");
			 if ($data){
			 	foreach ($data as $object){
			 		$array[] = (array)$object;
			 	}
    ...
    public function getCouponCustomerDiscountForWoo( $code='' )
    {
		...
		public function doesCouponExists( $type='', $search='' )
		{
				...
				switch ( $type ){
				 ...
				 case 'edd':
					 $table = $wpdb->prefix . 'postmeta';
					 $exists = $wpdb->get_var("SELECT meta_value
						 														FROM $table
												 								WHERE
				 ...
				 case 'ump':
					 $table = $wpdb->prefix . 'ihc_coupons';
					 $exists = $wpdb->get_var("SELECT code FROM $table
												 								WHERE code='$search'; ");
					 return $exists;
				}
				return false;
		...
		public function get_all_affiliates_slug($limit=0, $offset=0, $count=FALSE){
			 ...
			 */
			 ...
			 $array = array();
			 if ($count){
			 	$data = $wpdb->get_row("SELECT COUNT(umeta_id) as v FROM $table WHERE meta_key='uap_affiliate_custom_slug' AND meta_value<>'';");
		...
		public function get_magic_feat_item_list(){
			...
			$list = array(
					...
					'periodically_reports' => array(
							...
							'description' => __('Affiliates will receive periodic email reports about their stats', 'uap'),
					...
					'pushover_referral_notifications' => array(
							...
							'extra_class' => '',
							'description' => __('Admin receives notifications on mobile via Pushover when a Referral is created', 'uap'),
					...
					'info_affiliate_bar'	=> array(
							...
							'extra_class' => '',
							'description' => __('The quickest and easiest way to link to any page.', 'uap'),
					...
					'product_links'				=> array(
							...
							'extra_class' => '',
							'description' => __( 'Get fast affiliate link for all product available in your website.', 'uap' ),
			...
			$list = apply_filters( 'uap_magic_feature_list', $list );
			 ...
			 $list[ 'new_extension' ] = array(
                'label'						=> __( 'Add new Extensions', 'uap' ),
				 	...
				 	switch ($data->source){
						...
						case 'User SignUp':
							if ($this->is_magic_feat_enable('sign_up_referrals')){
								...
								$table = $wpdb->base_prefix . 'users';
								$exists = $wpdb->get_row("SELECT `ID`,`user_login`,`user_pass`,`user_nicename`,`user_email`,`user_url`,`user_registered`,`user_activation_key`,`user_status`,`display_name` FROM $table WHERE ID=$uid;");
								if (!empty($exists)){
									$value = TRUE;
								}
							}
					...
					switch ($temp_meta['uap_affiliate_payment_type']){
						...
						case 'bt':
							$return['uap_affiliate_bank_transfer_data'] = array(
																					'label' => __('Direct Deposit Details', 'uap'),
		...
		public function account_page_save_custom_menu_item($post_data=array())
		{
				if ($post_data && !empty($post_data['slug'])){
						...
						if (isset($data[$post_data['slug']])){
								...
								update_option( $key, @$post_data['url'] );
						}
				}
				return false;
		}
		...
		public function getLandingPagesForAffiliate($affiliateId=0)
    {
		...
		public function getEPCdata($type='', $affiliateId=0)
		{
				...
				$table = $wpdb->prefix . 'uap_visits';
				$q = "SELECT COUNT(id)  FROM $table WHERE affiliate_id=$affiliateId  AND UNIX_TIMESTAMP(visit_date)<=$today";
				if (!empty($start_time)){
					$q .= " AND UNIX_TIMESTAMP(visit_date)>$start_time ";
				}
		...
		public function saveIabUserSettings( $uid=0, $saveData=[] )
		{
				...
				foreach ( $data as $key=>$value ){
						if ( isset( $saveData[$key] ) ){
								update_user_meta( $uid, $key, esc_sql($saveData[$key]) );
						}
				}
		}
		...
		public function getCategoriesForServiceType( $type='' )
		{
			...
			{
					global $wpdb;
					return $wpdb->get_var( "SELECT count(id) FROM {$wpdb->prefix}uap_affiliates;");
			}
