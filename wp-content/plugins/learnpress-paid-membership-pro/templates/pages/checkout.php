...
<div id="pmpro_level-<?php echo $pmpro_level->id; ?>">
	<form id="pmpro_form" class="pmpro_form" action="<?php if ( ! empty( $_REQUEST['review'] ) ) {
		...
		<table id="pmpro_pricing_fields" class="pmpro_checkout" width="100%" cellpadding="0" cellspacing="0" border="0">
			...
			<tr>
				<th>
					<span class="pmpro_thead-name"><?php _e( 'Membership Level', 'learnpress-paid-membership-pro' ); ?></span>
					...
					<?php if ( $pmpro_show_discount_code ) { ?>
						...
						<?php if ( $discount_code && ! $pmpro_review ) { ?>
							<p id="other_discount_code_p" class="pmpro_small">
								<a id="other_discount_code_a" href="#discount_code"><?php _e( 'Click here to change your discount code', 'learnpress-paid-membership-pro' ); ?></a>.
							</p>
						<?php } elseif ( ! $pmpro_review ) { ?>
							<p id="other_discount_code_p" class="pmpro_small"><?php _e( 'Do you have a discount code?', 'learnpress-paid-membership-pro' ); ?>
								<a id="other_discount_code_a" href="#discount_code"><?php _e( 'Click here to enter your discount code', 'learnpress-paid-membership-pro' ); ?></a>.
							</p>
						<?php } elseif ( $pmpro_review && $discount_code ) { ?>
							<p>
								<strong><?php _e( 'Discount Code', 'learnpress-paid-membership-pro' ); ?>:</strong> <?php echo $discount_code ?>
							</p>
						<?php } ?>
			...
			<?php if ( $pmpro_show_discount_code ) { ?>
				<tr id="other_discount_code_tr" style="display: none;">
					<td>
						<div>
							<label for="other_discount_code"><?php _e( 'Discount Code', 'learnpress-paid-membership-pro' ); ?></label>
							...
							<input type="button" name="other_discount_code_button" id="other_discount_code_button" value="<?php _e( 'Apply', 'learnpress-paid-membership-pro' ); ?>" />
						</div>
					</td>
				</tr>
			<?php } ?>
		...
		<?php if ( ! $skip_account_fields && ! $pmpro_review ) { ?>
			<table id="pmpro_user_fields" class="pmpro_checkout" width="100%" cellpadding="0" cellspacing="0" border="0">
				...
				<tr>
					<th>
						<span class="pmpro_thead-name"><?php _e( 'Account Information', 'learnpress-paid-membership-pro' ); ?></span>
						<span class="pmpro_thead-msg"><?php _e( 'Already have an account?', 'learnpress-paid-membership-pro' ); ?>
							<a href="<?php echo wp_login_url( pmpro_url( "checkout", "?level=" . $pmpro_level->id ) ) ?>"><?php _e( 'Log in here', 'learnpress-paid-membership-pro' ); ?></a></span>
					</th>
				</tr>
				</thead>	
				<tbody>
				<tr>
					<td>
						<div>
							<label for="username"><?php _e( 'Username', 'learnpress-paid-membership-pro' ); ?></label>
						...
						<div>
							<label for="password"><?php _e( 'Password', 'learnpress-paid-membership-pro' ); ?></label>
						...
						if ( $pmpro_checkout_confirm_password ) {
							?>
							<div>
								<label for="password2"><?php _e( 'Confirm Password', 'learnpress-paid-membership-pro' ); ?></label>
						...
						<div>
							<label for="bemail"><?php _e( 'E-mail Address', 'learnpress-paid-membership-pro' ); ?></label>
						...
						if ( $pmpro_checkout_confirm_email ) {
							?>
							<div>
								<label for="bconfirmemail"><?php _e( 'Confirm E-mail Address', 'learnpress-paid-membership-pro' ); ?></label>
		...
		<?php } elseif ( $current_user->ID && ! $pmpro_review ) { ?>
			...
			<p id="pmpro_account_loggedin">
				<?php printf( __( 'You are logged in as <strong>%s</strong>. If you would like to use a different account for this membership, <a href="%s">log out now</a>.', 'learnpress-paid-membership-pro' ), $current_user->user_login, wp_logout_url( $_SERVER['REQUEST_URI'] ) ); ?>
			</p>
		<?php } ?>
		...
		<?php if ( pmpro_getGateway() == "paypal" && empty( $pmpro_review ) && true == apply_filters( 'pmpro_include_payment_option_for_paypal', true ) ) { ?>
			<table id="pmpro_payment_method" class="pmpro_checkout top1em" width="100%" cellpadding="0" cellspacing="0" border="0" <?php if ( ! $pmpro_requirebilling ) { ?>style="display: none;"<?php } ?>>
				<thead>
				<tr>
					<th><?php _e( 'Choose your Payment Method', 'learnpress-paid-membership-pro' ); ?></th>
				</tr>
				</thead>
				<tbody>
				<tr>
					<td>
						...
						<span class="gateway_paypal">
							...
							<a href="javascript:void(0);" class="pmpro_radio"><?php _e( 'Check Out with a Credit Card Here', 'learnpress-paid-membership-pro' ); ?></a>
						</span>
							...
							<a href="javascript:void(0);" class="pmpro_radio"><?php _e( 'Check Out with PayPal', 'learnpress-paid-membership-pro' ); ?></a>
						</span>
						</div>
					</td>
				</tr>
		...
		if ( $pmpro_include_billing_address_fields ) {
			...
			<table id="pmpro_billing_address_fields" class="pmpro_checkout top1em" width="100%" cellpadding="0" cellspacing="0" border="0" <?php if ( ! $pmpro_requirebilling || apply_filters( "pmpro_hide_billing_address_fields", false ) ){ ?>style="display: none;"<?php } ?>>
				<thead>
				<tr>
					<th><?php _e( 'Billing Address', 'learnpress-paid-membership-pro' ); ?></th>
				</tr>
				</thead>
				<tbody>
				<tr>
					<td>
						<div>
							<label for="bfirstname"><?php _e( 'First Name', 'learnpress-paid-membership-pro' ); ?></label>
						...
						<div>
							<label for="blastname"><?php _e( 'Last Name', 'learnpress-paid-membership-pro' ); ?></label>
						...
						<div>
							<label for="baddress1"><?php _e( 'Address 1', 'learnpress-paid-membership-pro' ); ?></label>
						...
						<div>
							<label for="baddress2"><?php _e( 'Address 2', 'learnpress-paid-membership-pro' ); ?></label>
						...
						if ( $longform_address ) {
							?>
							<div>
								<label for="bcity"><?php _e( 'City', 'learnpress-paid-membership-pro' ); ?></label>
							...
							<div>
								<label for="bstate"><?php _e( 'State', 'learnpress-paid-membership-pro' ); ?></label>
							...
							<div>
								<label for="bzipcode"><?php _e( 'Postal Code', 'learnpress-paid-membership-pro' ); ?></label>
							...
							?>
							<div>
								<label for="bcity_state_zip"><?php _e( 'City, State Zip', 'learnpress-paid-membership-pro' ); ?></label>
						...
						if ( $show_country ) {
							?>
							<div>
								<label for="bcountry"><?php _e( 'Country', 'learnpress-paid-membership-pro' ); ?></label>
						...
						?>
						<div>
							<label for="bphone"><?php _e( 'Phone', 'learnpress-paid-membership-pro' ); ?></label>
						...
						<?php if ( $skip_account_fields ) { ?>
							...
							if ( $pmpro_checkout_confirm_email ) {
								?>
								<div>
									<label for="bconfirmemail"><?php _e( 'Confirm E-mail', 'learnpress-paid-membership-pro' ); ?></label>
									<input id="bconfirmemail" name="bconfirmemail" type="<?php echo( $pmpro_email_field_type ? 'email' : 'text' ); ?>" class="input <?php echo pmpro_getClassForField( "bconfirmemail" ); ?>" size="30" value="<?php echo esc_attr( $bconfirmemail ) ?>" />
		...
		if ( $pmpro_include_payment_information_fields ) {
			...
			<table id="pmpro_payment_information_fields" class="pmpro_checkout top1em" width="100%" cellpadding="0" cellspacing="0" border="0" <?php if ( ! $pmpro_requirebilling || apply_filters( "pmpro_hide_payment_information_fields", false ) ) { ?>style="display: none;"<?php } ?>>
				...
				<tr>
					<th>
						<span class="pmpro_thead-name"><?php _e( 'Payment Information', 'learnpress-paid-membership-pro' ); ?></span>
					...
					<td>
						...
						if ( $pmpro_include_cardtype_field ) {
							...
							<div class="pmpro_payment-card-type">
								<label for="CardType"><?php _e( 'Card Type', 'learnpress-paid-membership-pro' ); ?></label>
						...
						<div class="pmpro_payment-account-number">
							<label for="AccountNumber"><?php _e( 'Card Number', 'learnpress-paid-membership-pro' ); ?></label>
						...
						<div class="pmpro_payment-expiration">
							<label for="ExpirationMonth"><?php _e( 'Expiration Date', 'learnpress-paid-membership-pro' ); ?></label>
						...
						if ( $pmpro_show_cvv ) { ?>
							<div class="pmpro_payment-cvv">
								<label for="CVV"><?php _e( 'CVV', 'learnpress-paid-membership-pro' ); ?></label>
								...
								<small>(<a href="javascript:void(0);" onclick="javascript:window.open('<?php echo pmpro_https_filter( PMPRO_URL ) ?>/pages/popup-cvv.html','cvv','toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=yes, resizable=yes, width=600, height=475');"><?php _e( "what's this?", 'learnpress-paid-membership-pro' ); ?></a>)
								</small>
							</div>
						<?php } ?>
						...
						<?php if ( $pmpro_show_discount_code ) { ?>
							<div class="pmpro_payment-discount-code">
								<label for="discount_code"><?php _e( 'Discount Code', 'learnpress-paid-membership-pro' ); ?></label>
								...
								<input type="button" id="discount_code_button" name="discount_code_button" value="<?php _e( 'Apply', 'learnpress-paid-membership-pro' ); ?>" />
		...
		<div class="pmpro_submit">
			<?php if ( $pmpro_review ) { ?>
				...
				<input type="submit" class="pmpro_btn pmpro_btn-submit-checkout" value="<?php _e( 'Complete Payment', 'learnpress-paid-membership-pro' ); ?> &raquo;" />
			</span>
			...
			<?php } else { ?>
				...
				if ( $pmpro_checkout_default_submit_button ) {
					...
					<input type="submit" class="pmpro_btn pmpro_btn-submit-checkout" value="<?php if ( $pmpro_requirebilling ) {
						_e( 'Submit and Check Out', 'learnpress-paid-membership-pro' );
					} else {
						_e( 'Submit and Confirm', 'learnpress-paid-membership-pro' );
					} ?> &raquo;" />
				</span>
					<?php
				}
			...
			<?php
			$processing_message = apply_filters( "pmpro_processing_message", __( "Processing...", 'learnpress-paid-membership-pro' ) );
