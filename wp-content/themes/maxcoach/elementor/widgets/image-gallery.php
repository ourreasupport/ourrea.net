...
class Widget_Image_Gallery extends Base {
	...
	public function get_title() {
		return esc_html__( 'Image Gallery', 'maxcoach' );
	}
	...
	private function add_content_section() {
		...
		$this->add_control( 'gallery', [
			'label'      => esc_html__( 'Add Images', 'maxcoach' ),
	...
	private function add_layout_section() {
		...
		$this->add_control( 'lightbox_enable', [
			'label'        => esc_html__( 'Lightbox', 'maxcoach' ),
			'type'         => Controls_Manager::SWITCHER,
			'return_value' => '1',
		] );
	...
	protected function render() {
		...
		$this->add_render_attribute( 'grid-wrapper', 'class', 'maxcoach-grid-wrapper' );
		...
		if ( ! empty( $settings['lightbox_enable'] ) ) {
			$this->add_render_attribute( 'grid-wrapper', 'class', 'maxcoach-light-gallery' );
		}
		?>
	...
	private function print_image( $image, $image_size ) {
		...
		if ( ! empty( $settings['lightbox_enable'] ) ) {
			...
			$image_full = \Maxcoach_Image::get_attachment_info( $image['id'] );
