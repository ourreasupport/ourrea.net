 ...
 */
if ( ! class_exists( 'Maxcoach_Customize' ) ) {
	class Maxcoach_Customize {
		...
		function initialize() {
			 ...
			 *
			 * @see Maxcoach_Global->init_global_variable()
			 */
		...
		function setup_override_settings() {
			...
			if ( ! is_customize_preview() ) {
				$presets = apply_filters( 'maxcoach_page_meta_box_presets', array() );

				if ( ! empty( $presets ) ) {
					foreach ( $presets as $preset ) {
						$page_preset_value = Maxcoach_Helper::get_post_meta( $preset );
			...
			if ( empty( self::$override_settings ) ) {
				 ...
				 */
				$selected_presets = apply_filters( 'maxcoach_settings_preset', array() );
		...
		public function add_custom_font_css() {
			$typo_fields = Maxcoach_Kirki::get_typography_fields_id();
			...
			if ( ! empty( $fonts ) ) {
				...
				foreach ( $fonts as $font ) {
					if ( strpos( $font, 'Gilroy' ) !== false ) {
						wp_enqueue_style( 'font-gilroy', MAXCOACH_THEME_URI . '/assets/fonts/gilroy/font-gilroy.css', null, null );
					} else {
						do_action( 'maxcoach_enqueue_custom_font', $font ); // hook to custom do enqueue fonts
					}
				}
			}
		}
		...
		public function customize_preview_css() {
			wp_enqueue_style( 'kirki-custom-css', MAXCOACH_THEME_URI . '/assets/admin/css/customizer.min.css' );
		}
		...
		public function load_customizer() {
			require_once MAXCOACH_CUSTOMIZER_DIR . '/customizer.php';
		}
		...
		function field_social_networks_enable( $args = array() ) {
			$defaults = array(
				...
				'label'       => esc_html__( 'Social Networks', 'maxcoach' ),
				'description' => '<a href="javascript:wp.customize.section( \'socials\' ).focus();">' . esc_html__( 'Edit social network links', 'maxcoach' ) . '</a>',
				'default'     => '0',
				'choices'     => array(
			...
			Maxcoach_Kirki::add_field( 'theme', $args );
		}
		...
		function field_social_sharing_enable( $args = array() ) {
			$defaults = array(
				...
				'description' => '<a href="javascript:wp.customize.section( \'social_sharing\' ).focus();">' . esc_html__( 'Edit social sharing list', 'maxcoach' ) . '</a>',
				'choices'     => array(
					'0' => esc_html__( 'Hide', 'maxcoach' ),
					'1' => esc_html__( 'Show', 'maxcoach' ),
				),
		...
		function field_language_switcher_enable( $args = array() ) {
			$defaults = array(
				'type'        => 'radio-buttonset',
				'label'       => esc_html__( 'Language Switcher', 'maxcoach' ),
				'description' => '',
				'choices'     => array(
					'0' => esc_html__( 'Hide', 'maxcoach' ),
					'1' => esc_html__( 'Show', 'maxcoach' ),
	...
	Maxcoach_Customize::instance()->initialize();
}
