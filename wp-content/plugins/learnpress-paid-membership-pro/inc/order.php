...
class LP_Pmpro_Order {
	...
	public function __construct() {
		...
		LP_Pmpro_Order::$_LIMIT_COURSES              = apply_filters( 'learn-press/pmspro/limit-courses', 10 );
		LP_Pmpro_Order::$_LIMIT_ORDERS               = apply_filters( 'learn-press/pmspro/limit-orders', 10 );
		LP_Pmpro_Order::$_CALL_CRON_JOB_AFTER_SECOND = apply_filters( 'learn-press/pmspro/time-call-cron-job-after-second', 5 );

		//add_action( 'template_include', array( $this, 'template_include' ), 15 ); # comment by tungnx - reason not use
		add_filter( 'learn_press_display_payment_method_title', array( $this, 'display_payment_method_title' ), 10, 2 );

		add_action( 'pmpro_after_checkout', array( $this, 'create_order_membership_success' ), 10, 2 ); // creat order membership success
		add_action( 'pmpro_membership_post_membership_expiry', array( $this, 'cronjob_user_membership_expire' ), 10, 2 );
		add_action( 'pmpro_before_change_membership_level', array( $this, 'before_update_lp_orders_when_change_membership_level' ), 11, 4 );
		add_action( 'pmpro_after_change_membership_level', array( $this, 'update_lp_orders_when_change_membership_level' ), 11, 3 );

		add_action( 'hook_single_event_add_courses_to_lp_order', array( $this, 'handleAddItemsToLpOrderCronjob' ), 10, 1 );
		add_action( 'hook_single_event_add_courses_to_lp_orders_when_save_level', array( $this, 'handleAddItemsToLpOrdersCronjobWhenCoursesLevelChange' ), 10, 1 );
	...
	public function auto_update_lp_orders() {
		...
		if ( ! empty( $user_orders ) ) {
			foreach ( $user_orders as $user_level_id => $user_level_orders ) {
				...
				if ( ! $hasMembershipLevel ) {
					...
					foreach ( $user_level_orders as $lp_order_id ) {
						learn_press_auto_enroll_user_to_courses( $lp_order_id );
					}
				}
			}
		}
		...
		foreach ( $user_levels as $user_level ) {
			...
			if ( $hasMembershipLevel ) {
				...
				if ( ! $lp_orders_data ) {
					# create new lp_order
					$lp_order_id = $this->create_lp_order_bk( $user_id, $user_level_id );
					if ( $lp_order_id ) {
						$this->redirect = true;
					}
	...
	public function create_lp_order_bk( $user_id = null, $level_id ) {
		# get course by level
			...
			case 'background':
				...
				$paramsCronjob = array(
					'params' => array(
						...
						'p'           => 0,
						'user_id'     => $user_id,
						'limit'       => LP_Pmpro_Order::$_LIMIT_COURSES,
					)
				);
				wp_schedule_single_event( time() + LP_Pmpro_Order::$_CALL_CRON_JOB_AFTER_SECOND, 'hook_single_event_add_courses_to_lp_order', $paramsCronjob );
	...
	public function get_total_courses_by_level( $level_id = 0 ) {
		...
		$sql = $wpdb->prepare(
			"SELECT COUNT(p.ID) as total
					...
					AND p.post_type = %s
					AND p.post_status = 'publish'",
			self::$_key_lp_pmpro_levels, $level_id, LP_COURSE_CPT );
	...
	public function handleAddItemToLpOrderCurl( $params ) {
		...
		$courses            = LpMbsProDb::getInstance()->getCoursesByLevel( $params['level_id'], $params['p'], $params['limit'] );
		...
		foreach ( array_keys( $courses ) as $course_id ) {
			...
			$curl        = LpMbsProHandleCurl::curl( 'POST', $params_send, $headers );

			array_push( $curlArr, $curl );
			curl_multi_add_handle( $curlMultiProcess, $curl );
		}

		LpMbsProHandleCurl::curlMultipleExec( $curlMultiProcess, $curlArr, function ( $response ) use ( &$total_item_success ) {
	...
	private function addItemsToLpOrder( $params = array(), $new_course_ids = array() ) {
		...
		foreach ( $new_course_ids as $course_id ) {
			...
			# Update user item
			$result_update_user_item_field = learn_press_update_user_item_field( array(
				'user_id'    => $user_id,
				'item_id'    => $course_id,
	...
	public function cronjob_user_membership_expire( $user_id, $membership_id ) {
		...
		$result = LpMbsProDb::getInstance()->getLastOrderMembershipOfUser( $user_id, $membership_id );
		...
		*/
		...
		if ( $result ) {
			...
			LpMbsProDb::getInstance()->updateStatusCoursesByOrderAndUser( $user_id, $order_id, $course_item_status );
		}
	}

	public function before_update_lp_orders_when_change_membership_level( $level_id, $user_id, $old_levels = array(), $cancel_level ) {
		if ( ! empty( $old_levels ) ) {
	...
	public function update_lp_orders_when_change_membership_level( $level_id = 0, $user_id = 0, $cancel_level = 0 ) {
		...
		if ( ! empty( $cancel_level ) ) { // Cancel level
			...
			if ( ! $order_id ) {
				debugLogLpPms( 'Error create new lp order' );
			}
	...
	public function LpOrderCancel( $user_id, $cancel_level_id ) {
		...
		$lpOrder = LpMbsProDb::getInstance()->getLastOrderMembershipOfUser( $user_id, $cancel_level_id );
		...
		if ( $lpOrder && isset( $lpOrder->post_id ) ) {
			...
			$resultStatusLpOrderItems = LpMbsProDb::getInstance()->updateStatusCoursesByOrderAndUser( $user_id, $lpOrder->post_id, 'cancelled' );
	...
	public function deleteCoursesOnOrders( $order_ids = array(), $del_course_ids = array() ) {
		// get user_item_ids
		$user_item_ids = LpMbsProDb::getInstance()->getUserItemIds( $order_ids, $del_course_ids );

		// get user item ids
		if ( is_array( $user_item_ids ) && count( $user_item_ids ) > 0 ) {
			$user_item_ids = array_keys( $user_item_ids );

			$rsDeleteUserItemmeta = LpMbsProDb::getInstance()->deleteUserItemmeta( $user_item_ids );
			$rsDeleteUserItems    = LpMbsProDb::getInstance()->deleteUserItems( $user_item_ids );
		}

		// get order item ids
		$order_item_ids = LpMbsProDb::getInstance()->getOrderItemIds( $order_ids, $del_course_ids );

		if ( is_array( $order_item_ids ) && count( $order_item_ids ) > 0 ) {
			$order_item_ids = array_keys( $order_item_ids );

			$rsDeleteOrderItemmeta = LpMbsProDb::getInstance()->deleteOrderItemmeta( $order_item_ids );
			$rsDeleteOrderItems    = LpMbsProDb::getInstance()->deleteOrderItems( $order_item_ids );
		}
	}
	...
	public function deleteCoursesOnOrder( $order_id = 0, $del_course_ids = array() ) {
		// get user item ids
		$user_item_ids = LpMbsProDb::getInstance()->getUserItemIds( array( $order_id ), $del_course_ids );
		...
		// get order item ids
		$order_item_ids = LpMbsProDb::getInstance()->getOrderItemIds( array( $order_id ), $del_course_ids );
	...
	public function addCoursesToOrders( $lp_orders, $new_course_ids = array() ) {
		...
		switch ( $run_mode ) {
			case 'background':
				$totalPageOrder = ceil( $total_orders / LP_Pmpro_Order::$_LIMIT_ORDERS );
				...
				$paramsCronjob = array(
					'params' => array(
						...
						'p_order'          => 0,
						'limit'            => LP_Pmpro_Order::$_LIMIT_ORDERS,
					)
				);

				wp_schedule_single_event( time() + LP_Pmpro_Order::$_CALL_CRON_JOB_AFTER_SECOND, 'hook_single_event_add_courses_to_lp_orders_when_save_level', $paramsCronjob );
	...
	public function handleAddItemsToLpOrdersCronjobWhenCoursesLevelChange( $params = array() ) {
		...
		$end              = $params['p_order'] * LP_Pmpro_Order::$_LIMIT_ORDERS + LP_Pmpro_Order::$_LIMIT_ORDERS;
		$i                = $params['p_order'];
	...
	/*public function handleAddItemsToLpOrderCronjobWhenCoursesLevelChange( $params = array() ) {
		$end              = $params['p_course'] * LP_Pmpro_Order::$_LIMIT_COURSES + LP_Pmpro_Order::$_LIMIT_COURSES;
		$i                = $params['p_course'];
		...
		if ( isset( $params_rs['code'] ) && $params_rs['code'] == 1 ) {
			...
			if ( $params_rs['p_course'] < $params_rs['total_page_course'] ) {
				wp_schedule_single_event( time() + LP_Pmpro_Order::$_CALL_CRON_JOB_AFTER_SECOND, 'hook_single_event_add_courses_to_lp_order_when_save_level', $paramsCronjob );
			} else {
				++ $params_rs['p_order'];
	...
	public function pms_updated_order_will_add_lp_order( $pms_order ) {
		...
		$pmsOrder = LpMbsProDb::getInstance()->getPmsOrderById( $pms_order->id );
		...
		if ( is_object( $pmsOrder ) ) {
			if ( isset( $pms_order->user_id ) && isset( $pms_order->membership_id ) && isset( $pms_order->status ) ) {
				...
				if ( $pms_order->status == 'success' || $pms_order->status == 'completed' ) {
					...
					$this->update_lp_orders_when_change_membership_level( $pms_order->membership_id, $pms_order->user_id, $pms_order->membership_id );
				}
			}
		}
	}
	 ...
	 * @param int   $level_id
	 */
	public function process_update_courses_on_lp_orders_when_save_level( $lp_orders = array(), $level_course_ids = array(), $level_id = 0 ) {
		$run_mode_setting = LP()->settings->get( 'pmpro_run_mode', 'auto' );
		$total_orders     = count( $lp_orders );
	 ...
	 * @param int   $level_id
	 */
	public function update_courses_on_lp_orders_when_save_level( $lp_orders = array(), $level_course_ids = array(), $level_id = 0 ) {
		foreach ( $lp_orders as $lp_order ) {
			// Get course ids on Lp order
			$order_course_ids = LpMbsProDb::getInstance()->getCourseIdsOnLpOrder( $lp_order->order_id );
			...
			if ( count( $remove_course_ids ) > 0 ) {
				LP_Pmpro_Order::getInstance()->deleteCoursesOnOrder( $lp_order->order_id, $remove_course_ids );
			}
			...
			if ( count( $add_course_ids ) > 0 ) {
				...
				$param_results = LP_Pmpro_Order::getInstance()->addItemsToLpOrder( $params, $add_course_ids );
			}
		}
	}
