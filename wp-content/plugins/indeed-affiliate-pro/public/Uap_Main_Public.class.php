	...
	class Uap_Main_Public{
		...
		public function __construct(){
			...
			if (!empty($temp['uap_woo_account_page_enable']) && is_plugin_active('woocommerce/woocommerce.php')){
				require UAP_PATH . 'classes/Uap_Custom_Woo_Endpoint.class.php';
			...
			if (!empty($temp['uap_bp_account_page_enable']) && is_plugin_active('buddypress/bp-loader.php')){
				require UAP_PATH . 'classes/Uap_BP_Custom_Menu_item.class.php';
		...
		public function add_style_and_scripts(){
			...
			//wp_enqueue_style( 'uap_font_awesome', UAP_URL . 'assets/css/font-awesome.min.css');

			//wp_enqueue_style( 'uap_public_style', UAP_URL . 'assets/css/main_public.css');
			wp_enqueue_style( 'uap_public_style', UAP_URL . 'assets/css/main_public.min.css');

			//wp_enqueue_style( 'uap_templates', UAP_URL . 'assets/css/templates.css');
			wp_enqueue_style( 'uap_templates', UAP_URL . 'assets/css/templates.min.css');
			...
			//wp_register_script( 'uap-public-functions', UAP_URL . 'assets/js/public-functions.js', array(), 1.1 );
			wp_register_script( 'uap-public-functions', UAP_URL . 'assets/js/public-functions.min.js', array(), 1.1 );
			...
			wp_register_script( 'uap-select2', UAP_URL . 'assets/js/select2.min.js', array(), null );
			wp_register_script( 'uap-jquery_form_module', UAP_URL . 'assets/js/jquery.form.js', array(), null );
			wp_register_script( 'uap-jquery.uploadfile', UAP_URL . 'assets/js/jquery.uploadfile.min.js', array(), null);
		}
		...
		public function do_tracking(){
			 ...
			 * @return none
			 */
			require UAP_PATH . 'public/Uap_Tracking.class.php';
		...
		public function affiliate_print_register_form($attr=array()){
			...
			if ($this->is_admin){
				...
				require UAP_PATH . 'classes/Uap_Add_Edit_Affiliate.class.php';
				$args = array(
						'user_id' => false,
						'type' => 'create',
						'tos' => true,
		...
		public function affiliate_print_login_form($args=array()){
			 ...
			 * @return string
			 */
			require UAP_PATH . 'public/Uap_Login.class.php';
			if ($this->is_admin){
			...
			if (!empty($_REQUEST['uapaction'])){ //can be GET || POST
				switch ($_REQUEST['uapaction']){
					...
					case 'reset_pass':
						...
						require UAP_PATH . 'public/Uap_Reset_Password.class.php';
		...
		public function return_admin_info_message($type=''){
			 ...
			 */
			 ...
			 switch ($type){
			 	case 'login':
					$data['content'] = __('Loggin Form is not showing up when You\'re logged. Open the page into incognito window instead.', 'uap');
					break;
				case 'register':
					$data['content'] = __('Register Form is not showing up when You\'re logged. Open the page into incognito window instead.', 'uap');
					break;
				case 'account_page':
		...
		public function do_listing_affiliates($params=array()){
			...
			if (!class_exists('Uap_Listing_Affiliates')){
				require UAP_PATH . 'classes/Uap_Listing_Affiliates.class.php';
			}
