...
class Widget_Testimonial_Carousel extends Static_Carousel {
	...
	public function get_title() {
		return esc_html__( 'Testimonial Carousel', 'maxcoach' );
	}
	...
	private function add_layout_section() {
		...
		$this->add_control( 'style', [
			...
			'options'      => [
				...
				'04' => esc_html__( '04', 'maxcoach' ),
				'05' => esc_html__( '05', 'maxcoach' ),
				'06' => esc_html__( '06', 'maxcoach' ),
				'07' => esc_html__( '07', 'maxcoach' ),
				'08' => esc_html__( '08', 'maxcoach' ),
				'09' => esc_html__( '09', 'maxcoach' ),
				'10' => esc_html__( '10', 'maxcoach' ),
			],
			'render_type'  => 'template',
			'prefix_class' => 'maxcoach-testimonial-style-',
		] );
		...
		$this->add_control( 'layout', [
			...
			'options'      => [
				'image-inline'  => esc_html__( 'Image Inline', 'maxcoach' ),
				'image-stacked' => esc_html__( 'Image Stacked', 'maxcoach' ),
				'image-top'     => esc_html__( 'Image Top Overlap', 'maxcoach' ),
				'image-top-02'  => esc_html__( 'Image Top', 'maxcoach' ),
				'image-above'   => esc_html__( 'Image Above', 'maxcoach' ),
				'image-left'    => esc_html__( 'Image Left', 'maxcoach' ),
			],
			'render_type'  => 'template',
			'prefix_class' => 'layout-',
		] );
		...
		$this->add_control( 'image_position', [
			'label'        => esc_html__( 'Info Position', 'maxcoach' ),
			'type'         => Controls_Manager::CHOOSE,
			'label_block'  => false,
			'default'      => 'below',
			'options'      => [
				'above'  => [
					'title' => esc_html__( 'Above', 'maxcoach' ),
					'icon'  => 'eicon-v-align-top',
				],
				'below'  => [
					'title' => esc_html__( 'Below', 'maxcoach' ),
		...
		$this->add_control( 'cite_layout', [
			'label'        => esc_html__( 'Cite Layout', 'maxcoach' ),
			'label_block'  => false,
			'type'         => Controls_Manager::CHOOSE,
			'default'      => 'block',
			'options'      => [
			...
			'prefix_class' => 'maxcoach-testimonial-cite-layout-',
		] );
	...
	private function add_content_style_section() {
		...
		$this->add_control( 'quote_icon_heading', [
			'label'     => esc_html__( 'Quote Icon', 'maxcoach' ),
	...
	protected function add_repeater_controls( Repeater $repeater ) {
		...
		$repeater->add_control( 'rating', [
			'label' => esc_html__( 'Rating', 'maxcoach' ),
	...
	protected function update_slider_settings( $settings, $slider_settings ) {
		if ( 'image-above' === $settings['layout'] ) {
			$slider_settings['class'][]            = 'maxcoach-main-swiper';
	...
	private function get_testimonial_rating_template( $rating = 5, $args = array() ) {
		$defaults = [
			'show_details' => false,
		];
		...
		$output = '<div class="testimonial-rating-wrap">';

		$output .= $this->get_rating_stars();

		if ( $args['show_details'] ) {
			$output .= '<div class="testimonial-rating-average">' . $this->number_format_nice_float( $rating ) . '/5</div>';
		}
	 ...
	 */
	private function number_format_nice_float( $number ) {
		// Not type.
		if ( 0 == $number ) {
			return $number;
		}
	...
	private function get_rating_stars( $rating = 5 ) {
	...
	private function print_testimonial_cite() {
		...
		if ( ! empty( $slide['rating'] ) && ! in_array( $settings['style'], [ '08' ] ) ):
	...
	protected function print_slide() {
		...
		<div <?php $this->print_attributes_string( $item_key . '-testimonial' ); ?>>
			...
			<?php if ( in_array( $settings['style'], [ '07' ], true ) ) : ?>
				<div class="testimonial-quote-icon">
					<?php echo \Maxcoach_Helper::get_file_contents( MAXCOACH_THEME_DIR . '/assets/images/testimonial-icon-02.svg' ); ?>
				</div>
			<?php elseif ( in_array( $settings['style'], [ '08' ], true ) ) : ?>
				<div class="testimonial-quote-icon">
					<?php echo \Maxcoach_Helper::get_file_contents( MAXCOACH_THEME_DIR . '/assets/images/testimonial-icon-03.svg' ); ?>
				</div>
			<?php endif; ?>
	...
	private function print_layout() {
		...
		<?php if ( ! empty( $slide['rating'] ) && in_array( $settings['style'], [ '08' ] ) ) : ?>
			<?php echo $this->get_testimonial_rating_template( $slide['rating'], [ 'show_details' => true ] ); ?>
		<?php endif; ?>
	...
	protected function before_slider() {
		...
		$this->add_render_attribute( '_wrapper', 'class', 'maxcoach-swiper-linked-yes' );

		$testimonial_thumbs_template = '';

		foreach ( $settings['slides'] as $slide ) :
			if ( $slide['image']['url'] ) :
				$testimonial_thumbs_template .= '<div class="swiper-slide"><div class="post-thumbnail"><div class="image">' . \Maxcoach_Image::get_elementor_attachment( [
						'settings'       => $slide,
						'image_size_key' => 'image_size',
					] ) . '</div></div></div>';
			endif;
		...
		?>
		<div class="tm-swiper tm-slider-widget maxcoach-testimonial-pagination style-01 maxcoach-thumbs-swiper"
		     data-lg-items="3"
		     data-lg-gutter="30"
