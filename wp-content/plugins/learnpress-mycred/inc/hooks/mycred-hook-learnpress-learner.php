...
if ( ! class_exists( 'myCred_LearnPress_Learner ' ) ) {
	...
	class myCred_LearnPress_Learner extends myCRED_Hook {
		...
		public function run() {
			...
			add_action( 'learn_press_update_order_status', array( $this, 'enroll_course' ), 10, 2 );
		 ...
		 * @param $order_id
		 */
		public function enroll_course( $status, $order_id ) {
			...
			if ( $items = $order->get_items() ) {
				foreach ( $items as $item ) {
					...
					if ( ! $this->over_hook_limit( $take_course, 'learnpress_learner' . '_' . $take_course, $user_id ) ) {
						$this->core->add_creds(
							'learnpress_learner' . '_' . $take_course, $user_id, $this->prefs[ $take_course ]['creds'], $this->prefs[ $take_course ]['log'], $course_id, array( 'ref_type' => 'post' ), $this->mycred_type
						);
					}
				}
			}
		...
		public function pass_course( $course_id, $user_id, $result ) {
			...
			if ( ! $this->over_hook_limit( 'pass_course', 'learnpress_learner_pass_course', $user_id ) ) {
				$this->core->add_creds(
					'learnpress_learner_pass_course', $user_id, $this->prefs['pass_course']['creds'], $this->prefs['pass_course']['log'], $course_id, array( 'ref_type' => 'post' ), $this->mycred_type
				);
			}
		}
