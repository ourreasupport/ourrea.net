...
class LearnPress_PMPro_Admin {
	...
	public function pmpro_save_membership_level( $level_id = 0 ) {
		...
		$courses        = LpMbsProDb::getInstance()->getCoursesOnLevel( $level_id );
		...
		$lp_orders = LpMbsProDb::getInstance()->getLastOrderOfUsersHasLevel( $level_id );

		if ( is_array( $lp_orders ) && count( $lp_orders ) > 0 ) {
			LP_Pmpro_Order::getInstance()->process_update_courses_on_lp_orders_when_save_level( $lp_orders, $level_course_ids, $level_id );
		}
		/***** End update orders *****/
	}
