...
class Widget_Full_Page extends Base {
	...
	public function __construct($data = [], $args = null) {
		...
		wp_register_style( 'maxcoach-widget-fullpage', MAXCOACH_THEME_URI . '/assets/libs/fullpage/fullpage.min.css', null, '3.0.5' );

		// Fullpage.
		wp_register_script( 'fullpage', MAXCOACH_THEME_URI . '/assets/libs/fullpage/fullpage.js', array(), null, true );
		//wp_register_script( 'fullpage-extensions', MAXCOACH_THEME_URI . '/assets/libs/fullpage/fullpage.extensions.min.js', array(), null, true );
		wp_register_script( 'scrolloverflow', MAXCOACH_THEME_URI . '/assets/libs/fullpage/vendors/scrolloverflow.js', array(), null, true );
		wp_register_script( 'maxcoach-widget-fullpage', MAXCOACH_ELEMENTOR_URI . '/assets/js/widgets/widget-fullpage.js', array(
			'elementor-frontend',
			'fullpage',
			'scrolloverflow',
		), null, true );
	...
	public function get_title() {
		return esc_html__( 'Fullpage', 'maxcoach' );
	}
	...
	public function get_script_depends() {
		return [ 'maxcoach-widget-fullpage' ];
	}
			...
			<?php if ( $settings['slides'] && count( $settings['slides'] ) > 0 ) {
				foreach ( $settings['slides'] as $key => $slide ) {
					...
					<div <?php $this->print_attributes_string( $item_key ); ?>>
						<?php echo Plugin::elementor()->frontend->get_builder_content_for_display( $slide['template_id'] ); ?>
					</div>
					<?php
				}
			}
	...
	private function add_slides_section() {
		...
		$repeater->start_controls_tab( 'repeater_slides_content_tab', [ 'label' => esc_html__( 'Content', 'maxcoach' ) ] );
		...
		$repeater->add_control( 'anchor', [
			'label' => esc_html__( 'Anchor', 'maxcoach' ),
			'type'  => Controls_Manager::TEXT,
		] );

		$repeater->add_control( 'skin', [
			'label'       => esc_html__( 'Skin', 'maxcoach' ),
			'description' => esc_html__( 'Controls the skin of pagination & header when view this section.', 'maxcoach' ),
			'type'        => Controls_Manager::CHOOSE,
			'options'     => [
				'light' => [
					'text' => esc_html__( 'Light', 'maxcoach' ),
				],
				'dark'  => [
					'text' => esc_html__( 'Dark', 'maxcoach' ),
				],
			],
			'toggle'      => false,
			'default'     => 'light',
		...
		$repeater->add_control( 'auto_height', [
			'label' => esc_html__( 'Auto Height', 'maxcoach' ),
			'type'  => Controls_Manager::SWITCHER,
		] );

		$repeater->end_controls_tab();

		$repeater->start_controls_tab( 'repeater_slides_background_tab', [ 'label' => esc_html__( 'Background', 'maxcoach' ) ] );
		...
		$this->add_control( 'slides', [
			...
			'default'     => [
				[
					'title' => esc_html__( 'Slide #1', 'maxcoach' ),
				],
				[
					'title' => esc_html__( 'Slide #2', 'maxcoach' ),
				],
			],
			'title_field' => '{{{ title }}}',
		] );
	...
	private function add_fullpage_options_section() {
		$this->start_controls_section( 'fullpage_options_section', [
			'label' => esc_html__( 'Fullpage Options', 'maxcoach' ),
		] );

		$this->add_control( 'dots', [
			'label' => esc_html__( 'Dots Enable', 'maxcoach' ),
			'type'  => Controls_Manager::SWITCHER,
		] );

		$this->add_control( 'dots_alignment', [
			'label'     => esc_html__( 'Dots Alignment', 'maxcoach' ),
			'type'      => Controls_Manager::CHOOSE,
			'options'   => [
				'left'  => [
		...
		$this->add_control( 'numbers', [
			'label' => esc_html__( 'Numbers Enable', 'maxcoach' ),
			'type'  => Controls_Manager::SWITCHER,
		] );

		$this->add_control( 'sharing', [
			'label' => esc_html__( 'Share Enable', 'maxcoach' ),
			'type'  => Controls_Manager::SWITCHER,
		] );
	...
	private function print_share( array $settings ) {
		...
		<div id="full-page-sharing" class="full-page-share-list">
			<div class="stalk">
				<?php esc_html_e( 'Stalk', 'maxcoach' ); ?>
			</div>
			<?php \Maxcoach_Templates::get_sharing_list( array(
	...
	private function print_slide_numbers( array $settings ) {
		...
		<div id="full-page-numbers" class="full-page-numbers">
			...
			<div class="title">
				<?php esc_html_e( 'Projects', 'maxcoach' ); ?>
			</div>
		</div>
		<?php
	}
