...
class Widget_Attribute_List extends Base {
	...
	public function get_title() {
		return esc_html__( 'Attribute List', 'maxcoach' );
	}
	...
	protected function _register_controls() {
		...
		$repeater->add_control( 'name', [
			...
			'default'     => esc_html__( 'Name', 'maxcoach' ),
			'label_block' => true,
		] );

		$repeater->add_control( 'value', [
			'label' => esc_html__( 'Value', 'maxcoach' ),
		...
		$this->add_control( 'items', [
			...
			'default'     => [
				[
					'name'  => esc_html__( 'Attribute #1', 'maxcoach' ),
					'value' => esc_html__( 'Value #1', 'maxcoach' ),
				],
				[
					'name'  => esc_html__( 'Attribute #2', 'maxcoach' ),
					'value' => esc_html__( 'Value #2', 'maxcoach' ),
				],
			],
			'title_field' => '{{{ name }}}',
		] );
					...
					<?php if ( $settings['items'] && count( $settings['items'] ) > 0 ) {
						foreach ( $settings['items'] as $key => $attribute ) {
							...
							<div class="item">
								...
								<div class="value"><?php echo wp_kses( $attribute['value'], 'maxcoach-default' ); ?></div>
							</div>
							<?php
						}
					}
